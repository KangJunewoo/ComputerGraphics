/**
 * Computer Graphics 과제 05
 * Author : 2016312924 강준우
 *
 * 기존 안드로이드 로봇 소스코드를 변형했다.
 * 오른쪽 클릭시 나왔던 기존 메뉴에 ★DANCE★를 추가했고,
 * 이를 누르면 청록색 로봇의 셔플댄스를 볼 수 있다.
 *
 * 나머지 사항들은 기존 소스코드와 동일하다.
*/
...
// 시간을 기록하기 위한 변수들. DANCE모드 시간을 기록하기 위해 time7을 추가했다.
...
static double time7 = 0; // dance_time 변수
...
static int flag = 0; // wireframe -solid rendering 모드 구분 변수
static int key = 0;  // 동작 모드와 색상 변경 변수

// 각각의 상수를 동작 이름으로 정의했고, 새로 DANCE를 추가함.
#define RUN		1					
#define JAP		2							
#define ROCKET	3	
#define YUNA	4	
...
void Draw_Color(int i) {
	...
	// DANCE 모드에 청록색 추가
	else if (i == DANCE) glColor3f(0.0, 0.8, 0.6);
}
...
void DrawL_Hand(int y, int a, int b, int c) {
	...
	// YUNA모드 혹은 DANCE모드일때 손을 깔때기 모양으로 함. 그게 아니면 기본 원기둥 모양.
	if (key == YUNA || key == DANCE) gluCylinder(cyl, 0.05, 0.02, 0.2, 15, 1);
...
void DrawR_Hand(int y, int a, int b, int c) {
	...
	// YUNA모드 혹은 DANCE모드일때 손을 깔때기 모양으로 함. 그게 아니면 기본 원기둥 모양.
	if (key == YUNA || key == DANCE) gluCylinder(cyl, 0.05, 0.02, 0.2, 50, 1);
...
//위의 함수들을 모두 이용해 안드로이드 로봇을 그리는 함수. DANCE모드 추가.
void DrawAndroid() {
	...
	else if (key == DANCE) DrawR_Hand(R_Arm_y, 0, -1, 0);
	else DrawR_Hand(R_Arm_y, 0, 0, 0);  //우측팔호출

	DrawL_Arm(L_Arm_x, 1, 0, 0);   //좌측팔뚝
	if (key == ROCKET) DrawL_HandRocket(); // 로켓모드일때 우측로켓호출
	else if (key == DANCE) DrawL_Hand(L_Arm_y, 0, 1, 0);
...
void Dance() {
	...
	glRotatef(sin(time7)* 10, 0, 0, 1);//로봇의 좌,우 반동 표현(좌우로 10도 만큼 주기적인 움직임 설정)

	glPushMatrix();// 처음 저장 좌표 다시 저장
	glTranslatef(0.0, 0.45, 0.0);//최초 위치 
	DrawAndroid();
	glutSwapBuffers();
}
...
void MyTimer(int Value) {
	...
	time7 += 0.3; // 춤출때 쓰이는 타이머 변수. 숫자가 클수록 빨라짐.
	glutPostRedisplay();
